...
<head>
    ...
    <title>${currentRm.mobileName}-<@spring.message "RemoteMobiles.Current.Title"/></title>
    <!-- title logo beg -->
...
<div class="page">
    <div class="page-header p-t-10 p-b-10">
        ...
        <ol class="breadcrumb">
            ...
            <@spring.message "RemoteMobiles.Current.ContextpathHome"/>
            </a></li>
            <li class="breadcrumb-item"><a href="${rc.contextPath}/realmobiles">
            <@spring.message "RemoteMobiles.Current.Contextpath"/>
            </a></li>
            <li class="breadcrumb-item active">${currentRm.mobileName}（<@spring.message "RemoteMobiles.Current.ContextpathTitle"/>）</li>
        </ol>
    </div>
    <div class="page-content">
        <div class="panel m-b-0">
            <div class="panel-heading">
                <h4 class="panel-title p-y-10">
                ${currentRm.mobileName}<span class="tag tag-default" id="remianTimeDaojishi">
                      <i class="icon wb-time"></i>
                      00:00:00
                    </span>
                    <input id="remainTime" type="hidden" value="${realMobile.remainTime}"/>

                    <span class="tag tag-success" id="signal"
                    <#if !currentRm.netSimulation??>
                          style="display: none"
                    </#if>
                    >
                        <i class="icon wb-signal">&nbsp;&nbsp;
                        <#if currentRm.netSimulation.systemDefault>
                        <@spring.message "RemoteMobiles.NetSimulation.key.${currentRm.netSimulation.key}"/>
                        <#else >
                        ${currentRm.netSimulation.key}
                        </#if>
                        </i>
                    </span>

                    <!--
                    ${currentRm.mobileName} （<@spring.message "RemoteMobiles.Current.RemainTime"/><span id="remianTimeDaojishi">00分00秒</span>）
                    <input id="remainTime" type="hidden" value="${realMobile.remainTime}"/>
                    <button type="button" id="btn_renewRm" data-rmid="${currentRm.id}" class="btn btn-success btn-xs">
                    <@spring.message "RemoteMobiles.Current.Btn.Extend"/>
                    </button>
                    <button type="button" id="btn_stopRm" data-rmid="${currentRm.id}" class="btn btn-danger btn-xs">
                    <@spring.message "RemoteMobiles.Current.Btn.Stop"/>
                    </button>
                    -->
                </h4>

                <!--<button type="button" class="btn btn-primary">停止调试</button>-->
                <div class="panel-actions">
                    <a href="javascript:void(0)" id="btn_renewRm" data-rmid="${currentRm.id}" class="panel-action icon wb-loop" title="<@spring.message "RemoteMobiles.Current.Btn.Extend"/>"></a>
                    <a href="javascript:void(0)" id="btn_stopRm" data-rmid="${currentRm.id}"class="panel-action icon wb-stop red-600" title="<@spring.message "RemoteMobiles.Current.Btn.Stop"/>"></a>
                    <a href="javascript:void(0)" class="panel-action icon wb-signal" data-toggle="dropdown" id="NetDropMenu" title="<@spring.message "RemoteMobiles.Current.Btn.NetSimulation"/>"></a>
                    <div class="dropdown-menu dropdown-menu-right" role="menu" aria-labelledby="NetDropMenu">
                        <#list weaknets as netSimulation>

                            <a href="javascript:void(0)" class="dropdown-item net_simulation"
                               data-rmid="${currentRm.id}"
                               data-nsId="${netSimulation.id}"
                               data-nsKey="${netSimulation.key}"
                               data-nsIsDefault="${netSimulation.sysdefault}"
                               role="menuitem"
                            >
                            <#if netSimulation.sysdefault>
                                <@spring.message "weaknet.type.default.${netSimulation.key}.name"/>
                            <#else >
                                ${netSimulation.key}
                            </#if>
                            </a>
                        ...
                        <a class="dropdown-item" href="javascript:void(0)" id="remove_net_simulation" data-rmid="${currentRm.id}" role="menuitem"><@spring.message "RemoteMobiles.Current.removeNetSimulation"/></a>
                    </div>
                    <a href="javascript:void(0)" class="panel-action icon wb-fullscreen" data-toggle="panel-fullscreen" aria-hidden="true"></a>
                </div>
            </div>
            <div class="panel-body p-t-0 p-x-5 p-b-5">
                <iframe style="width:100%; height:600px;" src="${realMobile.url}" id="myiframe" scrolling="no" frameborder="0"></iframe>
            </div>
        </div>
    </div>
</div>
...
<script src="${rc.contextPath}/static/js/realmobile/realmobile_one.js"></script>
